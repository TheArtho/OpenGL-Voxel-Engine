...
namespace Minecraft
{
    public class Shader : IDisposable
    {
        ...
        public unsafe void SetUniform(string name, Vector3[] value)
        {
            ...
            {
                //A new overload has been created for setting a uniform so we can use the transform in our shader.
                int location = _gl.GetUniformLocation(_handle, $"{name}[{i}]");
                if (location == -1)
                {
                    throw new Exception($"{name}[{i}] uniform not found on shader.");
                }
                _gl.Uniform3(location, (float) value[i].X, (float) value[i].Y, value[i].Z);
            }
        }
        ...
        private uint LoadShader(ShaderType type, string path)
        {
            ...
            {
                throw new Exception($"Error compiling shader of type {type} ({path}), failed with error {infoLog}");
            }
